...
const schema = z
  .object({
    name: z.string().min(3).max(50, "Nome muito grande"),
    ...
    telNumber: z.string().min(10).max(11),
  })
  .required();
...
const getSteps = (erros: string[]) => {
  return sourceSteps.map((step) => {
    return {
      ...step,
      hasError: erros.some((erro) => step.fields.includes(erro)),
    };
  });
};
